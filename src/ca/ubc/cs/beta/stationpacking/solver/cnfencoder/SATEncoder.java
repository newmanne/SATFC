...
import ca.ubc.cs.beta.stationpacking.solver.sat.Clause;
import ca.ubc.cs.beta.stationpacking.solver.sat.Litteral;
...
public class SATEncoder implements ISATEncoder {
	...
	{
		int a = (int) (z-Math.pow(Math.floor(Math.sqrt(z)),2));
		int b =(int) Math.floor(Math.sqrt(z));
		...
		{
			return new Pair<Integer,Integer>(b,a-b);
		}
