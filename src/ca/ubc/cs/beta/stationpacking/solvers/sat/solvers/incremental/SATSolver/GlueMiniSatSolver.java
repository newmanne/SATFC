...
public class GlueMiniSatSolver implements IIncrementalSATSolver{
	 ...
	 * variables are added.
	 */
	private Map<Long,Integer> fExternalToInternal = new HashMap<Long,Integer>();
	private Map<Integer,Long> fInternalToExternal = new HashMap<Integer,Long>();
	...
	private interface GMSLibrary extends Library {
		...
		public SATResult solve(Clause aAssumptions, double aCutOff){
			...
			for(Litteral aLitteral : aAssumptions)
			{
				fGMSsolver.addLitToVec(vecAssumptions, getInternalVariable(aLitteral.getVariable()), aLitteral.getSign());
			}
		...
		public boolean addClause(Clause aClause){
			...
			{
				fGMSsolver.addLitToVec(vecLiterals,getInternalVariable(aLitteral.getVariable()),aLitteral.getSign());
			}
		...
		public Clause getAssignment(){
			...
			if(fGMSsolver.okay(fSolverPointer)){
				for(int i = 0; i < fGMSsolver.nVars(fSolverPointer); i++){
					...
					aAssignment.add(new Litteral(fInternalToExternal.get(i),b));
				}
			}
			return aAssignment;
		}
		 ...
		 */
		private Integer getInternalVariable(long aVar){
		...
		*/
		...
		private void runBasicTests(){
	        ...
	        aClause = new Clause();
	        aClause.add(new Litteral(1,false));
	        aClause.add(new Litteral(2, true));
	        addClause(aClause);
	        aClause = new Clause();
	        aClause.add(new Litteral(2,false));
	        addClause(aClause);
	        aClause = new Clause();
	        printResult(aClause, 1); 
	        
	        System.out.println("Testing trivial UNSAT instance...");
	        aClause.add(new Litteral(1,true));
	        printResult(aClause, 1);
	        ...
	        aClause.add(new Litteral(1,true));
	        aClause.add(new Litteral(2,true));
	        aClause.add(new Litteral(3,true));
	        aClause.add(new Litteral(4,false));
	        addClause(aClause);
	        aClause = new Clause();
	        aClause.add(new Litteral(1,false));
	        aClause.add(new Litteral(2,false));
	        ...
	        aClause = new Clause();
	        aClause.add(new Litteral(1,false));
	        aClause.add(new Litteral(3,false));
	        aClause.add(new Litteral(4,false));
	        addClause(aClause);
	        aClause = new Clause();
	        aClause.add(new Litteral(2,false));
	        ...
	        aClause = new Clause();
	        aClause.add(new Litteral(4,true));
	        printResult(aClause, 1);
