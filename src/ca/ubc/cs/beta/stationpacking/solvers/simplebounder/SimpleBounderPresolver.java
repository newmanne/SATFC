package ca.ubc.cs.beta.stationpacking.solvers.simplebounder;
...
public class SimpleBounderPresolver implements ISolver{
	...
	public SolverResult solve(StationPackingInstance aInstance, double aCutoff,
		...
		{
			log.info("No assignment to use for bounding pre-solving.");
		...
		log.info("There are {} stations that are not part of previous assignment.",missingStations.size());
		//Check if there are too many stations to make this procedure worthwile.
		if(missingStations.size()>MAX_MISSING_STATIONS)
		{
			log.info("Too many missing stations in previous assignment ({}).",missingStations.size());
			
			return new SolverResult(SATResult.TIMEOUT, preWatch.stop()/1000.0);
		}
		
		log.info("Building constraint graph.");
		 ...
		 */
		log.info("Evaluating if stations not in previous assignment ({}) with their neighborhood are unpackable.",topackStations.size());
		...
		if(aCutoff-aTimeSpent>0)
		{
			...
			{
				log.info("Stations not in previous assignment cannot be packed with their neighborhood.");
		...
		log.info("Evaluating if stations not in previous assignment ({}) with their neighborhood are packable when all other stations are fixed to previous assignment.",reducedDomainStations.size());
		...
		if(aCutoff-aTimeSpent>0)
		{
			...
			{
				log.info("Stations not in previous assignment can be packed with their neighborhood when all other stations are fixed to their previous assignment..");
