...
class AbstractWrapper(object):
    ...
    def __init__(self):
        ...
        self.killed_by_runsolver = False
        ...
        self._unzipped_assignment_file = None
    ...
    def main(self, argv=None):
        ...
        try:
            ...
            # Watch out for prefix matching... (want the allow_abbrev=False option from python 3.5)
            ...
            if args.ext_callstring:
                target_cmd = self.get_command_line_args_ext(runargs=runargs, config=config_dict, ext_call=args.ext_callstring)
            else:
                target_cmd = self.get_command_line_args(runargs=runargs, config=config_dict)
    ...
    def call_target(self, target_cmd):
        ...
        try:
            if self._runsolver != "None":
                if "\"" in " ".join(map(str, runsolver_cmd)): # if there are quotes in the call, we cannot split it individual list elements. we have to call it via shell as a string; problematic solver: SparrowToRiss
    ...
    def read_runsolver_output(self):
        ...
        if self.killed_by_runsolver:
            print "Runsolver killed this run"
        ...
        if (cpu_match1 or cpu_match2):
            print "CPUTime is %f" % self._ta_runtime

        wall_time_pattern = re.compile('Real time \\(s\\): (%s)' % (self.float_regex()))
        wall_time_match = re.search(wall_time_pattern, data)

        if (wall_time_match):
            self._walltime = float(wall_time_match.group(1))
            print "Walltime is %f" % self._walltime
    ...
    def cleanup(self):
        ...
        if (self._ta_status is "ABORT" or self._ta_status is "CRASHED"):
            if (len(self._ta_misc) == 0):
                self._ta_misc = 'Problem with run. Exit code was %s.' % (str(self._ta_exit_code) if self._ta_exit_code else "not available")
        ...
        try:
            if (self._unzipped_file):
                self._unzipped_file.close()
            if (self._unzipped_assignment_file):
            ...
            # UBCSAT crashes too much, so let's not keep these around
            ...
            if not self.args.preserve:
                ...
                if (self._unzipped_file):
                    os.remove(self._unzipped_file.name)
                if (self._unzipped_assignment_file):
                    os.remove(self._unzipped_assignment_file.name)
        except (OSError, KeyboardInterrupt, SystemExit):
