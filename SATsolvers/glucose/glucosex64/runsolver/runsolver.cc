...
//	procTree->dumpProcessTree(cout);
	...
	*/
	...
	cout << "runsolver_walltime: " << realtime2/1000.0 << endl;
...
int main(int argc, char **argv)
{
  ...
  try
  {
      ...
      case 1005:
	{
	  // very basic parsing
	  int first,last;
	  if (sscanf(optarg,"%d-%d",&first,&last)!=2 || first>last)
	  {
	    cout << "Syntax: --cores first-last with first<=last" << endl;
	    exit(1);
	  }
    ...
    cout << "runsolver Copyright (C) 2010-2011 Olivier ROUSSEL\n"
	 << "\n"
    ...
    int childstatus = solver.run(&argv[optind]);
