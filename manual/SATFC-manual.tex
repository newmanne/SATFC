...
> bash compile.sh
\end{lstlisting}
Note that this version of \clasp requires a version of libtbb (\url{https://www.threadingbuildingblocks.org/}) that may not be part of your linux distribution by default - keep an eye on the output of the compile script.
...
The \SATFCServer uses a data structure which must know about all of the station IDs that it could ever possibly see when it starts up. To specify these IDs the server takes a parameter, \texttt{---stations.file}, which should specify a path to a file in which every line corresponds to a station ID. For most cases, it should suffice to create a file which is simply the second column of your \texttt{Domain.csv} (which lists all of the station IDs).

A ready-to-go \SATFC server can be found in the releases folder of \SATFC's root directory. You can start it by running
\begin{lstlisting}[style=Bash]
> ./SATFCServer --redis.host=<redishost> --redis.port=<redisport> --stations.file=<stationfilepath>
\end{lstlisting}
...
\item \texttt{---stations.file} A path to a file in which every line contains the id of a station
