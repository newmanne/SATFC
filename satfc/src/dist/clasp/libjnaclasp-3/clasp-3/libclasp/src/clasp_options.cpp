...
struct ClaspCliConfig::ParseContext : public ProgramOptions::ParseContext {
	...
	ParseContext() : root(0) {}
	void init() { 
		first = root->find("configuration"); 
		OptionIter it = root->find(HEU_PARAM);
		root->addAlias("vsids-decay", it);
		root->addAlias("berk-max", it);
		root->addAlias("vmtf-mtf", it);
	}
...
bool ClaspCliConfig::set(const ConfigIter& config, bool allowConfig, const ParsedOpts& exclude, ParsedOpts* out) {
	if (!opts_.get() || !opts_->root.get()) { init(new OptionContext(), true); }
...
const ClaspCliConfig::ParsedOpts& ClaspCliConfig::finalizeParsed(UserConfig* active, const ParsedOpts& parsed, ParsedOpts& exclude) const {
	...
	CLASP_FAIL_IF(parsed.count(HEU_PARAM) && !parsed.count("heuristic"), "'--" HEU_PARAM "' requires '--heuristic'");
	if (parsed.count("heuristic") && !parsed.count(HEU_PARAM)) {
		if (!copied) { exclude = parsed; copied = true; }
		exclude.add(HEU_PARAM);
		active->addSolver(0).heuParam = 0;
	}
...
bool ClaspCliConfig::setConfig(const RawConfig& config, ProblemType t) {
	ProgramOptions::ParsedOptions exclude;
	reset();
	return set(config.iterator(), true, exclude, &exclude) && opts_->root->assignDefaults(exclude) && finalize(exclude, t, true);
}
