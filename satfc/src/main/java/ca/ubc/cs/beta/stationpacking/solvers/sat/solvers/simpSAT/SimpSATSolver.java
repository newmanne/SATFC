package ca.ubc.cs.beta.stationpacking.solvers.sat.solvers.simpSAT;
 ...
 */
@Slf4j
public class SimpSATSolver extends AbstractCompressedSATSolver {

    private UBCSATLibrary fLibrary;
    private final String simpSATPath;
...
//    private final ProblemIncrementor problemIncrementor;
    ...
    public SimpSATSolver(String simpSATPath, String runsolverPath, String parameters, String nickname) {
//        log.info("Building SimpSATSolver");
        this.simpSATPath = simpSATPath;
...
//        log.info("Done Building SimpSATSolver");
    ...
    public SATSolverResult solve(CNF aCNF, Map<Long, Boolean> aPreviousAssignment, ITerminationCriterion aTerminationCriterion, long aSeed) {
        ...
        try {
            ...
            File simpSATDir = new File(simpSATPath);
            ...
            processString = processString + " ./simpsat --randomize=" + Long.toString(aSeed) + " ";
            processString = processString + parameters + tempIn.getCanonicalPath();
            Process pr = rt.exec(processString,null,simpSATDir);
            try {
            pr.waitFor();
